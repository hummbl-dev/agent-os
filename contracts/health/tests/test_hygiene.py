"""Tests: config hygiene and secret checks for health status."""
...
from .fixtures import VALID_MINIMAL_HEALTH
...
def test_sk_in_message_raises():
    payload = deepcopy(VALID_MINIMAL_HEALTH)
    payload["checks"][0]["message"] = "sk-fake-token"
...
def test_apikey_literal_injection_raises():
    payload = deepcopy(VALID_MINIMAL_HEALTH)
    payload["checks"][0]["details"] = {"apiKey": "literal-secret-value"}
